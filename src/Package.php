...
class Package
{
    ...
    private function deduceTrackingCode(): Package
    {
        ...
        } elseif (preg_match('/^[1-9]{1}[0-9]{11}$/', $tracking_code, $matches)) {
            $carrier_code = Carrier::CODE_FEDEX;
        ...
        } elseif (preg_match('/(\b\d{30}\b)|(\b91\d+\b)|(\b\d{20}\b)/', $tracking_code)) {
            $carrier_code = Carrier::CODE_USPS;
        } elseif (preg_match('/^E\D{1}\d{9}\D{2}$|^9\d{15,21}$/', $tracking_code)) {
            $carrier_code = Carrier::CODE_USPS;
        }
        elseif (preg_match('/^[A-Za-z]{2}[0-9]+US$/', $tracking_code)) {
            $carrier_code = Carrier::CODE_USPS;
        } elseif (preg_match(
